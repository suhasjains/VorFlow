...
#print np.linalg.norm((np.dot(L,data.press)+4.0*np.pi*np.pi*kx*kx/L_x/L_x*data.press)/N1)
print np.linalg.norm((np.dot(L,data.press)+(4.0*np.pi*np.pi*kx*kx/L_x/L_x+4.0*np.pi*np.pi*ky*ky/L_y/L_y)*data.press)/N1)
